...
const ShowCars = ({ car }) => {
  ...
  return (
    <div >
      ...
      <div className="h-[550px] mt-4 card card-compact w-96 bg-base-100 shadow-xl">
        <figure>
          <img src={img_url} className="h-[200px]" alt="Shoes" />
        </figure>
        <div className="card-body">
        <span>Category: {categoryName}</span>
          <h2 className="card-title">{car_name} <div className="badge badge-secondary">Price: ${resale_price}</div></h2>
        ...
        <p className="block text-xs font-medium tracking-widest uppercase dark:text-violet-400">Real-price: ${original_price}</p>
        <p>Used: {used_year}</p>
        </div>
